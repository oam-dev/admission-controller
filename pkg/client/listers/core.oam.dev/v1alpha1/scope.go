...
// List lists all Scopes in the indexer.
func (s *scopeLister) List(selector labels.Selector) (ret []*v1alpha1.Scope, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Scope))
	})
	return ret, err
}
...
// List lists all Scopes in the indexer for a given namespace.
func (s scopeNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Scope, err error) {
...
// Get retrieves the Scope from the indexer for a given namespace and name.
func (s scopeNamespaceLister) Get(name string) (*v1alpha1.Scope, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("scope"), name)
	}
	return obj.(*v1alpha1.Scope), nil
}
